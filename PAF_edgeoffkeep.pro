PRO PAF_edgeoffkeep, sname, Tsys_Y = Tsys_Y, Tsys_X = Tsys_X, chanRange = chanRange, order = order, badScans = badScans, raDecLimits = raDecLimits, fileout=outFile
;==============================================================================
...
for idx = 0,N_ELEMENTS(goodScans)-1 DO BEGIN
    ...
    scale, Tsys_Y*exp(0.01/sin(!g.s[0].elevation*3.14159/180.0))/0.99 ; Ta*, will default to buffer 0 
    ...
    divide, 6, 2, 0
    ; scale to units of Ta*
    scale, Tsys_X*exp(0.01/sin(!g.s[0].elevation*3.14159/180.0))/0.99 ; Ta*, will default to buffer 0
    ;; set region for baseline subtraction
    nregion,chanRange
    IF KEYWORD_SET(order) THEN baseline,nfit=order
    x=dcextract(!g.s[0],floor(chanRange[0]),ceil(chanRange[n_elements(chanRange)-1])) ; extracts spectrum over regions of interest
